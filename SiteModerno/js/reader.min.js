...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderReader(volId, filename, json, allFiles, searchQuery) {
        ...
        topicsFound.forEach((topicData, index) => {
            ...
            if (headerHTML) {
                const redundantSubMatch = rawContent.match(/^[\s\n]*(?:<(?:b|strong)>[\s\n]*)?(?:<font[^>]*>[\s\n]*)?(?:<(?:b|strong)>)?([^<]{2,150}?)(?:<\/(?:b|strong)>)?(?:[\s\n]*<\/font>[\s\n]*)?(?:<\/(?:b|strong)>[\s\n]*)?(?:<br\s*\/?>[\s\n]*)+/i);
                if (redundantSubMatch) {
                    let subTitle = redundantSubMatch[1].trim();
                    subTitle = subTitle.replace(/^\*\*|\*\*$/g, '');
                    const c1 = typeof cleanTitle === 'string' ? cleanTitle.toLowerCase().replace(/[^a-z0-9]/g, '') : '';
                    const c2 = subTitle.toLowerCase().replace(/[^a-z0-9]/g, '');
                    if (c1 && c2 && (c2.includes(c1) || c1.includes(c2))) {
                        rawContent = rawContent.substring(redundantSubMatch[0].length).replace(/^[\s\n]*<br\s*\/?>[\s\n]*/gi, '');
                    }
                }
            }
